...
def init_erp_db():
    ...
    # Enhanced Payroll with Government Compliance
    c.execute('''
        CREATE TABLE IF NOT EXISTS payslips (
            ...
            employee_code TEXT,
            employee_pan TEXT,
            employee_aadhar TEXT,
            employee_uan TEXT,
            employee_esi_number TEXT,
            designation TEXT,
            department TEXT,
            ...
            -- Compliance
            pf_applicable BOOLEAN DEFAULT 1,
            esi_applicable BOOLEAN DEFAULT 1,
            pt_applicable BOOLEAN DEFAULT 1,
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS payroll_rules (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            vendor_id INTEGER NOT NULL,
            rule_type TEXT NOT NULL CHECK(rule_type IN ('pf_rate', 'esi_rate', 'pt_rate', 'basic_salary_percentage', 'hra_percentage')),
    ...
    # Insert default payroll rules
    c.execute('''
        INSERT OR IGNORE INTO payroll_rules (vendor_id, rule_type, rule_value, effective_from) VALUES
        (1, 'pf_rate', 12.0, '2024-01-01'),
        (1, 'esi_rate', 3.25, '2024-01-01'),
        (1, 'pt_rate', 200.0, '2024-01-01'),
        (1, 'basic_salary_percentage', 50.0, '2024-01-01'),
        (1, 'hra_percentage', 40.0, '2024-01-01')
    ''')
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS payroll_accounting_entries (
            ...
            vendor_id INTEGER NOT NULL,
            account_type TEXT NOT NULL CHECK(account_type IN ('salary_expense', 'pf_liability', 'esi_liability', 'tds_liability', 'salary_payable')),
...
def generate_payslip(employee_id):
    ...
    # Get vendor ID
    c.execute("SELECT id, name, address, state, pincode FROM vendors WHERE email=?", (email,))
    ...
    vendor_id, company_name, company_address, company_state, company_pincode = vendor_result
    ...
    if request.method == "POST":
        ...
        c.execute("""
            SELECT e.*, ess.basic_salary, ess.hra, ess.conveyance_allowance, 
                   ess.medical_allowance, ess.special_allowance, ess.gross_salary
            FROM employees e
        ...
        hours_data = c.fetchone()
        total_hours = hours_data[0] or 0
        days_worked = hours_data[1] or 0

        # Calculate salary components
        basic_salary = employee_data[17] or employee_data[5]  # From structure or base salary
        hra = employee_data[18] or (basic_salary * 0.40)
        conveyance = employee_data[19] or 1600
        medical = employee_data[20] or 1250
        special = employee_data[21] or 0
        
        # Calculate performance bonus from revenue tracking
        c.execute("""
            SELECT COALESCE(SUM(commission_amount), 0) 
            FROM employee_revenue_tracking 
            WHERE employee_id = ? AND transaction_date BETWEEN ? AND ?
        """, (employee_id, pay_period_start, pay_period_end))
        
        performance_bonus = c.fetchone()[0] or 0

        # Calculate overtime (hours beyond 8 hours/day)
        overtime_hours = max(0, total_hours - (days_worked * 8))
        overtime_amount = overtime_hours * (employee_data[6] * 1.5)  # 1.5x hourly rate

        gross_salary = basic_salary + hra + conveyance + medical + special + performance_bonus + overtime_amount

        # Calculate deductions
        pf_employee = basic_salary * 0.12 if gross_salary >= 15000 else 0
        esi_employee = gross_salary * 0.0075 if gross_salary <= 25000 else 0
        professional_tax = 200 if gross_salary > 10000 else 0
        tds = max(0, (gross_salary * 12 - 250000) * 0.05 / 12) if gross_salary * 12 > 250000 else 0

        total_deductions = pf_employee + esi_employee + professional_tax + tds
        net_salary = gross_salary - total_deductions

        # Generate payslip number
        payslip_number = f"PS{vendor_id}{employee_id}{month_year.replace('-', '')}"

        # Insert payslip
        c.execute("""
            INSERT INTO payslips 
            (employee_id, vendor_id, payslip_number, pay_period_start, pay_period_end, month_year,
             company_name, company_address, designation, basic_salary, hra, conveyance_allowance,
             medical_allowance, special_allowance, performance_bonus, overtime_amount,
             gross_salary, pf_employee, esi_employee, professional_tax, tds, total_deductions,
             net_salary, days_worked, status, generated_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'generated', ?)
        """, (employee_id, vendor_id, payslip_number, pay_period_start, pay_period_end, month_year,
              company_name, f"{company_address}, {company_state} - {company_pincode}", 
              employee_data[4], basic_salary, hra, conveyance, medical, special, 
              performance_bonus, overtime_amount, gross_salary, pf_employee, esi_employee,
              professional_tax, tds, total_deductions, net_salary, days_worked, email))

        payslip_id = c.lastrowid

        # Create accounting entries
        # Salary Expense (Debit)
        c.execute("""
            INSERT INTO ledger_entries (vendor_id, entry_type, account, amount, description, sub_category)
            VALUES (?, 'debit', 'Salary Expense', ?, ?, 'Payroll')
        """, (vendor_id, gross_salary, f"Salary expense - {employee_data[1]} - {month_year}"))

        c.execute("""
            INSERT INTO payroll_accounting_entries (payslip_id, vendor_id, account_type, debit_amount, description)
            VALUES (?, ?, 'salary_expense', ?, ?)
        """, (payslip_id, vendor_id, gross_salary, f"Salary expense for {employee_data[1]}"))

        # Employee Liabilities (Credit)
        if pf_employee > 0:
            c.execute("""
                INSERT INTO ledger_entries (vendor_id, entry_type, account, amount, description, sub_category)
                VALUES (?, 'credit', 'PF Liability', ?, ?, 'Payroll')
            """, (vendor_id, pf_employee * 2, f"PF liability - {employee_data[1]} - {month_year}"))  # Employee + Employer

        if esi_employee > 0:
            c.execute("""
                INSERT INTO ledger_entries (vendor_id, entry_type, account, amount, description, sub_category)
                VALUES (?, 'credit', 'ESI Liability', ?, ?, 'Payroll')
            """, (vendor_id, esi_employee + (gross_salary * 0.0325), f"ESI liability - {employee_data[1]} - {month_year}"))

        if tds > 0:
            c.execute("""
                INSERT INTO ledger_entries (vendor_id, entry_type, account, amount, description, sub_category)
                VALUES (?, 'credit', 'TDS Liability', ?, ?, 'Payroll')
            """, (vendor_id, tds, f"TDS liability - {employee_data[1]} - {month_year}"))

        # Salary Payable (Credit)
        c.execute("""
            INSERT INTO ledger_entries (vendor_id, entry_type, account, amount, description, sub_category)
            VALUES (?, 'credit', 'Salary Payable', ?, ?, 'Payroll')
        """, (vendor_id, net_salary, f"Net salary payable - {employee_data[1]} - {month_year}"))

        # Add to expenses table for P&L reporting
        c.execute("""
            INSERT INTO expenses (vendor_id, category, amount, description, date)
            VALUES (?, 'Salary & Wages', ?, ?, ?)
        """, (vendor_id, gross_salary, f"Salary - {employee_data[1]} - {month_year}", pay_period_end))

        conn.commit()
        conn.close()

        flash(f"Payslip generated successfully for {employee_data[1]}")
        return redirect(url_for("view_payslip", payslip_id=payslip_id))

    # Get employee details for form
    c.execute("SELECT * FROM employees WHERE id = ? AND vendor_id = ?", (employee_id, vendor_id))
    employee = c.fetchone()

    conn.close()
    return render_template("generate_payslip.html", employee=employee)

@app.route('/erp/hr/payslip/<int:payslip_id>')
def view_payslip(payslip_id):
    ...
    c.execute("""
        SELECT p.*, e.name, e.email, e.phone, e.join_date, e.position
        FROM payslips p
        JOIN employees e ON p.employee_id = e.id
        WHERE p.id = ? AND p.vendor_id = ?
    """, (payslip_id, vendor_id))
    
    payslip_data = c.fetchone()
    if not payslip_data:
        flash("Payslip not found")
        return redirect(url_for("manage_employees"))

    conn.close()
    return render_template("payslip_view.html", payslip=payslip_data)

@app.route('/erp/hr/payslips')
def manage_payslips():
    if "vendor" not in session:
        return redirect(url_for("erp_login"))
    ...
    # Get all payslips with employee names
    c.execute("""
        SELECT p.id, p.payslip_number, p.month_year, p.gross_salary, p.net_salary, 
               p.status, p.generated_date, e.name as employee_name
        FROM payslips p
        JOIN employees e ON p.employee_id = e.id
        WHERE p.vendor_id = ?
    ...
    conn.close()
    return render_template("manage_payslips.html", payslips=payslips)

@app.route('/erp/hr/accounting-integration')
def hr_accounting_integration():
    if "vendor" not in session:
        return redirect(url_for("erp_login"))
    ...
    # Get HR-related accounting entries
    c.execute("""
        SELECT le.id, le.entry_type, le.account, le.amount, le.description, le.timestamp
        FROM ledger_entries le
        WHERE le.vendor_id = ? AND le.sub_category IN ('Payroll', 'HR')
        ORDER BY le.timestamp DESC
        LIMIT 50
    """, (vendor_id,))
    
    hr_ledger_entries = c.fetchall()

    # Get payroll expense summary
    c.execute("""
        SELECT 
            SUM(CASE WHEN account = 'Salary Expense' THEN amount ELSE 0 END) as total_salary_expense,
            SUM(CASE WHEN account = 'PF Liability' THEN amount ELSE 0 END) as total_pf_liability,
            SUM(CASE WHEN account = 'ESI Liability' THEN amount ELSE 0 END) as total_esi_liability,
            SUM(CASE WHEN account = 'TDS Liability' THEN amount ELSE 0 END) as total_tds_liability
        FROM ledger_entries
        WHERE vendor_id = ? AND sub_category = 'Payroll'
    """, (vendor_id,))
    
    payroll_summary = c.fetchone()

    conn.close()
    return render_template("hr_accounting_integration.html", 
                         hr_ledger_entries=hr_ledger_entries,
