...
def get_stats():
    ...
    try:
        ...
        # Get basic stats
        c.execute("SELECT COUNT(*) FROM vendors WHERE is_online = 1")
        online_vendors = c.fetchone()[0] or 0
        
        c.execute("SELECT COUNT(*) FROM products WHERE quantity > 0")
        available_products = c.fetchone()[0] or 0
        
        c.execute("SELECT COUNT(*) FROM bookings WHERE status = 'confirmed'")
        active_bookings = c.fetchone()[0] or 0
        
        c.execute("SELECT COALESCE(SUM(total_amount), 0) FROM sales_log WHERE sale_date >= date('now', '-7 days')")
        weekly_sales = c.fetchone()[0] or 0
